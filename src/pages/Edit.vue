...
import { ArrowLeft, Pencil, AlertCircle, CheckCircle } from 'lucide-vue-next';
          ...
          <div class="p-8">
            <div class="flex items-center gap-3 mb-8">
              <div class="w-10 h-10 rounded-xl bg-indigo-500/30 flex items-center justify-center">
                <Pencil class="w-5 h-5 text-indigo-300" />
              </div>
              <h1 class="text-2xl font-bold text-white">Edit Post</h1>
            </div>

            <form @submit.prevent="handleSubmit" class="space-y-6">
              <div v-if="error" class="bg-rose-500/10 border border-rose-400/20 text-rose-200 py-3 px-4 rounded-xl flex items-center gap-2 backdrop-blur-sm">
                <AlertCircle class="w-5 h-5 flex-shrink-0" />
                {{error}}
              </div>
              
              <div v-if="success" class="bg-emerald-500/10 border border-emerald-400/20 text-emerald-200 py-3 px-4 rounded-xl flex items-center gap-2 backdrop-blur-sm">
              ...
              <div class="flex items-center gap-3 pt-4">
                <button type="submit" class="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold py-3 px-6 rounded-xl transition-all shadow-lg shadow-indigo-500/30">
